// Durata: LEGATO-A:P03
/* ============================================================
   SI Parrucchieri — flusso di prenotazione guidato
...
function renderStep1() {
  ...
  SERVIZI.filter((s) => s.cat === stato.categoria).forEach((s) => {
    ...
    b.innerHTML = `
      ...
      <div class="costo">
        <span class="prezzo">${s.da ? "<small>da </small>" : ""}${euro(s.prezzo)}</span><br />
...
function renderStep2() {
  ...
  sotto.textContent = `Poche domande rapide su "${stato.servizio.nome}": così ${TEAM[0].nome} e il team arrivano preparati al tuo appuntamento.`;
  pannello.appendChild(sotto);

  quiz.forEach((q) => {
    const box = document.createElement("div");
    box.className = "domanda";
    const facolt = q.tipo === "testo" ? "" : "";
    box.innerHTML = `<span class="testo-domanda">${q.label}${facolt}</span>`;
    if (q.tipo === "scelta") {
...
function renderStep3() {
  pannello.innerHTML = intestazione(3, "Hai una foto del look che sogni?");
  const sotto = document.createElement("p");
  sotto.style.cssText = "color: var(--grigio); margin: -0.8rem 0 1.8rem;";
  sotto.textContent = "Facoltativo, ma utilissimo: allega fino a 3 foto trovate online (Instagram, Pinterest…) del taglio o colore che desideri. Il salone arriva preparato.";
  pannello.appendChild(sotto);
  ...
  if (stato.foto.length) {
    ...
    stato.foto.forEach((src, i) => {
      ...
      box.innerHTML = `<img src="${src}" alt="Foto di ispirazione ${i + 1}" />
  ...
  privacy.innerHTML = `🔒 <span><strong>Le foto restano private:</strong> le vede solo il salone, per preparare al meglio il tuo appuntamento.</span>`;
...
function renderStep6() {
  ...
  riepilogo.innerHTML = `
    <dl>
      ...
      <dt>Prezzo indicativo</dt><dd>${s.da ? "da " : ""}${euro(s.prezzo)}</dd>
  ...
  opzioni.innerHTML = `
    <button class="opzione-pag ${stato.pagamento === "caparra" ? "scelto" : ""}" data-pag="caparra">
      ...
      <span>
        ...
        <span>Il tuo appuntamento è <b>garantito</b>. La caparra verrà <b>scalata dal costo totale</b> del servizio.</span>
      </span>
    </button>
    <button class="opzione-pag ${stato.pagamento === "salone" ? "scelto" : ""}" data-pag="salone">
      <span class="radio"></span>
      <span>
        <strong>🏠 Paga tutto in salone</strong>
...
function renderConferma(p) {
  ...
  pannello.innerHTML = `
    <div class="conferma">
      ...
      ${p.pagamento === "caparra"
        ? `<p>Caparra di ${euro(p.caparra)} ricevuta ✓ — verrà scalata dal costo del servizio. Il tuo posto è garantito.</p>`
        : `<p>Pagherai direttamente in salone. Se non puoi venire, avvisaci al ${SALONE.telefono} 🙏</p>`}
      ${p.foto.length ? `<p>Abbiamo ricevuto le tue ${p.foto.length} foto di ispirazione: le vedrà solo il salone 📸</p>` : ""}
